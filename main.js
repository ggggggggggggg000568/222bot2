...
setTimeout(() => {
    try {
        const jidRaw = global.conn?.user?.id;
        const jidClean = jidRaw?.replace(/[:;].*$/, "") + "@s.whatsapp.net";

        console.log("=======================================");
        console.log(" BOT JID RAW:", jidRaw);
        console.log(" BOT JID NORMALIZZATO:", jidClean);
        console.log("=======================================");
    } catch (e) {
        console.log("Errore log JID bot:", e);
    }
}, 2000);
    ...
    if (opzione === '2' || methodCode) {
        ...
        if (!conn.authState.creds.registered) {
            ...
            if (phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.bold.bgCyanBright(`Inserisci il numero di WhatsApp.\n${chalk.bold.yellowBright("Esempio: +393471234567")}\n${chalk.bold.cyan('━━► ')}`)));
                addNumber = phoneNumber.replace(/\D/g, '');
                if (!phoneNumber.startsWith('+')) phoneNumber = `+${phoneNumber}`;
                rl.close();
            }
            setTimeout(async () => {
                let codeBot = await conn.requestPairingCode(addNumber, '222based');
                codeBot = codeBot?.match(/.{1,4}/g)?.join("-") || codeBot;
                console.log(chalk.bold.white(chalk.bgCyan('『 🔐 』– CODICE DI ABBINAMENTO:')), chalk.bold.white(chalk.white(codeBot)));
            }, 3000);
        }
    }
}
...
async function by222team() {
    try {
        const mainChannelId = global.IdCanale?.[0] || 'xxxxxxxxxxx@newsletter';
...
if (!opts['test']) {
    if (global.db) setInterval(async () => {
        ...
        if (opts['autocleartmp'] && (global.support || {}).find) {
            const tmp = [tmpdir(), 'tmp', "222bot-sub"];
...
async function connectionUpdate(update) {
    ...
    if (connection === 'open') {
        ...
        if (!global.isLogoPrinted) {
            const neonColors = [
                '#00FF88', '#00FFAA', '#00FFCC', '#00FFFF', '#00DDFF',
                '#00BBFF', '#0099FF', '#0077FF', '#0055FF', '#0033FF', '#0011FF'
            ];
            const botLogo = [
                ` ██████╗ ██████╗ ██████╗     ██████╗  ██████╗ ████████╗`,
                `██╔═══██╗██╔══██╗██╔══██╗    ██╔══██╗██╔═══██╗╚══██╔══╝`,
            ...
            botLogo.forEach((line, i) => {
                const color = neonColors[i] || neonColors[neonColors.length - 1];
    ...
    if (connection === 'close') {
        ...
        if (reason === DisconnectReason.badSession && !global.connectionMessagesPrinted.badSession) {
            ...
            console.log(chalk.bold.yellowBright(`╭✦❘༻ ⚠️ CONNESSIONE SOSTITUITA ⚠️ ༺❘✦╮\n┃  È stata aperta un'altra sessione, \n┃  chiudi prima quella attuale.\n╰✦❘༻ 𝟮𝟮𝟮 𝗕𝗢𝗧 ༺❘✦╯`));
...
async function connectSubBots() {
    const subBotDirectory = './222bot-sub';
    if (!existsSync(subBotDirectory)) {
        console.log(chalk.bold.cyanBright('⚡ 222 BOT non ha Sub-Bot collegati. Creazione directory...'));
        try {
            mkdirSync(subBotDirectory, { recursive: true });
            console.log(chalk.bold.green('✅ Directory 222bot-sub creata con successo.'));
        } catch (err) {
            console.log(chalk.bold.red('❌ Errore nella creazione della directory 222bot-sub:', err.message));
            return;
        }
        return;
    }
    ...
    try {
        ...
        if (subBotFolders.length === 0) {
            console.log(chalk.bold.cyan('- ⚡ | Nessun subbot collegato'));
            return;
        }
        ...
        if (global.conns.length > 0) {
            console.log(chalk.bold.cyanBright(`⚡ ${global.conns.length} Sub-Bot si sono connessi con successo.`));
        } else {
...
(async () => {
    ...
    try {
        ...
        console.log(chalk.bold.cyan(`✦❘༻ ⚡ 222 BOT CONNESSO CORRETTAMENTE ⚡ ༺❘✦`));
...
const mainWatcher = watch(filePath, async () => {
  console.log(chalk.bold.bgCyanBright("File: 'based.js' Aggiornato"));
  await global.reloadHandler(true).catch(console.error);
});
