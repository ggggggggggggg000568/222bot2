...
global.decodeJid = (jid) => {
    ...
    // Gestione LID (nuovo formato) - preserva il JID completo
    if (jid.includes(':') && jid.includes('@')) {
        const [userPart, serverPart] = jid.split('@');
        const [phoneNumber, device] = userPart.split(':');
        jid = `${phoneNumber}@${serverPart}`;
    }
...
// Aggiungi anche normalizeJid per completezza
global.normalizeJid = (jid) => {
    return global.decodeJid(jid).toLowerCase();
};
